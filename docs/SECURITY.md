...
This project has **no versioned releases**. Everything on `main` is deployed:
a push to `main` runs CI, then `wrangler deploy` for all three Workers
(`apps/web`, `apps/dashboard`, `apps/api`) — see `.github/workflows/ci.yml`
and `docs/deployment.md`.

| Source | Supported |
| ------ | --------- |
| `main` (latest deployed state) | ✅ |
| Anything else (tags, forks, local branches) | ❌ — reproduce on `main` before reporting |

Security fixes land on `main` and are live as soon as the deploy job finishes.
There is no backport process.
