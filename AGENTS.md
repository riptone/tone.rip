...
- **[docs/architecture.md](./docs/architecture.md)** - how the 3 apps and 6 packages fit together, and why things live where they do. Read this before deciding whether something belongs in a package or an app.
...
- `bun run ci` runs the entire CI pipeline locally in order (format, lint, check-types, test, build, check-bundle-size, e2e, knip, check-cycles, the generated-CV drift check) - the one command to run before considering a change done.
- `bun run test` enforces per-package coverage thresholds (`vitest.config.ts`'s `coverage.thresholds`), `bun run check-bundle-size` guards each Worker's bundled-script size, and `bun run e2e` (Playwright, `packages/config/playwright-config`) smoke-tests `apps/web`/`apps/dashboard` against a real `wrangler dev` - all three run in CI; see [docs/deployment.md](./docs/deployment.md) for the bundle-size budget.
