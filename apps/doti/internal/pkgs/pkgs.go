...
type Runner interface {
	...
	// Output runs a command for what it says rather than for what it does,
	// and returns its stdout.
	//
	// Separate from Run because the two want opposite things from stdout:
	// Run captures it as evidence for an error and otherwise discards it,
	// while this is called precisely to read it. See owned.go, which asks a
	// package manager for its inventory.
...
// Output returns the command's stdout, with stderr kept back for the error.
//
...
func (r ExecRunner) Output(ctx context.Context, name string, args ...string) ([]byte, error) {
	...
	invocation := name + " " + strings.Join(args, " ")
	if tail.String() == "" {
		return out, fmt.Errorf("%s: %w", invocation, err)
	}
...
// Brewfile renders the Homebrew bundle for everything the manifest declares.
//
...
func Brewfile(m *manifest.Manifest) string {
	return BrewfileOf(m.Tools, m.ZshPlugins, m.BrewCasks)
}

// BrewfileOf renders the bundle for exactly the three lists it is given.
//
// One renderer rather than one per subset. There used to be two - the whole
// manifest, and a tools-only file for `--tools` - and the moment the selector
// could tick individual tools that became a trap: rendering the tools-only file
// because one tool was unticked would silently drop every cask and zsh plugin
// too, so declining `bat` also declined Ghostty. Each list is narrowed by its
// own caller, and nothing here infers one from another.
func BrewfileOf(tools []manifest.Tool, plugins []manifest.ZshPlugin, casks []manifest.Cask) string {
...
// WingetPackages renders the `winget import` file for everything the manifest
// declares.
func WingetPackages(m *manifest.Manifest) (string, error) {
	return WingetPackagesOf(m.Tools, m.WingetExtras)
}

// WingetPackagesOf renders it for exactly the two lists it is given, so a
// selection narrows what gets imported - the Windows half of BrewfileOf.
func WingetPackagesOf(tools []manifest.Tool, extras []string) (string, error) {
	source := wingetSource{}
